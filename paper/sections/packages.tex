...
\medskip
{\footnotesize\emph{Note:} Council tax is abolished in every row. Revenue above the \pounds 58.5 billion replacement is divided equally across all individuals. No benefit interactions are modelled on the dividend.py}.}
\end{table}
...
\medskip
{\footnotesize\emph{Note:} Each row replaces the same \pounds 58.5 billion. ``Untaxed'' includes the 17.2 per cent of households holding no land.py}.}
\end{table}
...
\medskip
{\footnotesize\emph{Note:} Stamp duty is modelled as an annual flow of liabilities on the transactions occurring in the simulation year, and replacing it with a recurrent charge therefore redistributes from movers to holders in a way this static comparison captures only in the first year.py}.}
\end{table}
