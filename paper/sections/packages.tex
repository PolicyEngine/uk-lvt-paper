...
\medskip
{\footnotesize\emph{Note:} Council tax is abolished in every row. Revenue above the \pounds 58.5 billion replacement is divided equally across all individuals. No benefit interactions are modelled on the dividend.}
\end{table}
...
\medskip
{\footnotesize\emph{Note:} Each row replaces the same \pounds 58.5 billion. ``Untaxed'' includes the 17.2 per cent of households holding no land.}
\end{table}
...
\medskip
{\footnotesize\emph{Note:} Stamp duty is modelled as an annual flow of liabilities on the transactions occurring in the simulation year, and replacing it with a recurrent charge therefore redistributes from movers to holders in a way this static comparison captures only in the first year.}
\end{table}
...
None of this makes the surcharge a bad measure --- it raises revenue from an under-taxed group at low administrative cost, and it establishes that the Valuation Office can value high-value homes at current prices, which is the precondition for anything more ambitious. But it operates at roughly one per cent of the scale of the reform considered here, and it leaves the underlying band structure, and hence the regressivity documented in Table~\ref{tab:etr}, unchanged.
