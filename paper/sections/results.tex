...
Land value is the most spatially concentrated tax base in the UK, so the reform redistributes across places at least as much as across households. Figure~\ref{fig:region_effect} reports household-weighted average effects for the nine English statistical regions and three devolved nations, using the same microdata and 0.79 per cent budget-neutral rate as the household tables. Wales gains \pounds 608 per household on average, Scotland \pounds 435, the North East \pounds 420, and Yorkshire and the Humber \pounds 393. London loses \pounds 1{,}125 per household, while the East of England and South East record much smaller average losses of \pounds 96 and \pounds 83. Northern Ireland loses \pounds 1{,}026 because the central specification adds LVT without abolishing domestic rates. The regional aggregate changes sum to zero, as required by UK-wide revenue neutrality.

\begin{figure}[htbp]
\centering
\safefig{../results/figures/fig9_net_change_by_region.png}
\caption{Household-weighted average net income change by UK region at the 0.79 per cent budget-neutral rate. \emph{Note:} positive values indicate an average gain. Northern Ireland's domestic rates are unchanged in the central specification. \emph{Source:} Authors' calculations using PolicyEngine UK 2.89.4 and Enhanced FRS 2023--24 release 1.56.14; \texttt{analysis/figures.py}.}
\label{fig:region_effect}
\end{figure}

Figure~\ref{fig:map} maps the average net income change by Westminster constituency.\footnote{The constituency weights available to us are calibrated against a later release of the microdata than the one used elsewhere in the paper, so this map is computed on that release, with the rate re-solved for revenue neutrality on it (0.69 rather than 0.79 per cent). The pattern is comparable; the levels are not directly comparable with the tables above.}
