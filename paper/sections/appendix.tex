...
\quad Households & 32.08m & Weighted EFRS household count. This exceeds ONS household estimates (roughly 28.6m for 2023) because the upstream EFRS weight calibration targets person-level and administrative totals rather than household counts; per-household averages and the modelled liability totals use this denominator, and the gap is one candidate source of the council tax overshoot against OBR receipts discussed in Section~\ref{sec:methodology} \\
\addlinespace
